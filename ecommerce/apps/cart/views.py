...
from apps.product.serializers import ProductSerialiazer
...
class UpdateItemView(APIView):
    def put(self, request, format=None):
        ...
        try:
            ...
            return Response({'error':'Product ID no exist'}, status=status.HTTP_404_NOT_FOUND)
        
        try:
            count = int(data['count'])
        except:
            return Response({'error':'Count ID no exist'}, status=status.HTTP_404_NOT_FOUND)
        ...
        try:
            ...
            if not CartItem.objects.filter(cart=cart, product=product).exists():
                return Response({'error':'Este Producto no esta en tu carro'}, status=status.HTTP_404_NOT_FOUND)
    ...
    def delete(self, request, format=None):
        ...
        try:
            ...
            if not CartItem.objects.filter(cart=cart).exists():
                return Response({'succes': 'Cart esta vacio'}, status=status.HTTP_200_OK)
